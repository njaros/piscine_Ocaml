...
let string_of_protein protein =
  let rec recu lst str =
    ...
    | h::t -> (
      match h with
      | Stop -> recu t (str ^ "End of transaction")
      | Ala -> recu t (str ^ "Alanine ")
      | Arg -> recu t (str ^ "Arginine ")
      | Asn -> recu t (str ^ "Asparagine ")
      | Asp -> recu t (str ^ "Aspatique ")
      | Cys -> recu t (str ^ "Cysteine ")
      | Gln -> recu t (str ^ "Glutamine ")
      | Glu -> recu t (str ^ "Glutamine ")
      | Gly -> recu t (str ^ "Glycine ")
      | His -> recu t (str ^ "Histidine ")
      | Ile -> recu t (str ^ "Isoleucine ")
      | Leu -> recu t (str ^ "Leucine ")
      | Lys -> recu t (str ^ "Lysine ")
      | Met -> recu t (str ^ "Methionine ")
      | Phe -> recu t (str ^ "Phenylalanine ")
      | Pro -> recu t (str ^ "Proline ")
      | Ser -> recu t (str ^ "Serine ")
      | Thr -> recu t (str ^ "Threonine ")
      | Trp -> recu t (str ^ "Tryptophane ")
      | Tyr -> recu t (str ^ "Tyrosine ")
      | Val -> recu t (str ^ "Valine ")
    )
  in
  (recu protein "") ^ "\n"
...
let life () =
  let nb = Random.int (500) in
  print_string "let's have an helix with ";
  print_int nb;
  print_endline " nucleotids.";
  let helix = generate_helix nb in
  print_endline "helix generated.";
  let arn = generate_rna helix in
  print_endline "rna of this helix generated.";
  print_endline "the rna has those protein : ";
  let protein_string = string_of_protein (decode_arn arn) in
  print_string protein_string
